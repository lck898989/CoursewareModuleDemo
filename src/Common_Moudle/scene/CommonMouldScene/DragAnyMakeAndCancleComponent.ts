class DragAnyMakeAndCancleComponent extends eui.Component{
    ...
    public ImgList:Array<eui.Image>;
    ...
    {   
        ...
        if(this.curMoveItemIndex >= -1 && this.curMoveItemObj){
            ...
            let isCenSet = this.rectCanSet.hitTestPoint(stageX, stageY)
    ...
    { 
        if (data["reset" + this.curComponentIndex.toString()]){
            ...
            let keyData = data["touchBegin" + this.curComponentIndex.toString()]
            let isNewItem:string = keyData["isNewItem"];
            let curIndexStr:string = keyData["curIndexStr"];
            let targetX:number = parseInt(keyData["targetX"]);
            let targetY:number = parseInt(keyData["targetY"]);
            this.revStartHandle(isNewItem, curIndexStr, targetX, targetY);
        } else if(data["touchMove" + this.curComponentIndex.toString()]){
            let keyData = data["touchMove" + this.curComponentIndex.toString()]
            let targetX:number = parseInt(keyData["targetX"]);
            let targetY:number = parseInt(keyData["targetY"]);
            this.revMoveHandle(targetX, targetY);
        } else if(data["touchEndLayer" + this.curComponentIndex.toString()]){
            let keyData = data["touchEndLayer" + this.curComponentIndex.toString()]
            let isCancelStr:string = keyData["isCancelStr"];
            let isNewItemStr:string = keyData["isNewItemStr"];
            let targetX:number = parseInt(keyData["targetX"]);
            let targetY:number = parseInt(keyData["targetY"]);
            let deleteIndex:number = parseInt(keyData["deleteIndex"]);
