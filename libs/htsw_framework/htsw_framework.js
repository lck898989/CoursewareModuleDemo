...
var HttpConnect = (function () {
    ...
    HttpConnect.prototype.onPostComplete = function (event) {
        ...
        Log.trace("msg", "回来消息用时= " + (egret.getTimer() - request.time));
        Log.trace("msg", "消息：" + JSON.stringify(request.response));
        if (request.hander) {
            if (request.isJson) {
    ...
    HttpConnect.prototype.onPostIOError = function (event) {
        Log.trace("com", "post error : " + event);
    };
    /**
     *
     */
    HttpConnect.prototype.onPostProgress = function (event) {
        Log.trace("com", "post progress : " + Math.floor(100 * event.bytesLoaded / event.bytesTotal) + "%");
    };
    return HttpConnect;
}());
...
var JsonManager = (function () {
    ...
    JsonManager.prototype.onResourceLoadError = function (event) {
        Log.trace("msg", event);
        Log.trace("json", "json文件加载失败 = " + event.groupName);
    };
...
var ResLoad = (function () {
            ...
            if (_callBack) {
                if (!_callBack.done) {
                    if (!_callBack.loaded) {
                        ...
                        Log.trace("msg", "==资源组加载完成执行回调==" + _callBack.name);
                        if (_handler) {
    ...
    ResLoad.prototype.onResourceLoadComplete = function (event) {
        Log.trace("msg", "资源组 " + event.groupName + " 加载完成");
    ...
    ResLoad.prototype.onResourceLoadError = function (event) {
        Log.trace("msg", "图片资源加载错误" + event);
    };
    ResLoad.prototype.onResourceProgress = function (event) {
        Log.trace("msg", "onResourceProgress:" + event.groupName);
        Log.trace("msg", "onResourceProgress_event.itemsLoaded:" + event.itemsLoaded);
        Log.trace("msg", "onResourceProgress_event.itemsTotal:" + event.itemsTotal);
...
var SceneManager = (function () {
    ...
    SceneManager.prototype.runScene = function (classFactory, resName, data) {
        if (data === void 0) { data = null; }
        Log.trace("msg", "运行前场景========" + classFactory.key + "====" + this._loadingScene);
    ...
    SceneManager.prototype.loadEnd = function (group) {
        ...
        if (curScene) {
            if (!GameLayerManager.gameLayer().sceneLayer.contains(curScene)) {
                ...
                Log.trace("msg", "画布中已经有该场景:" + this._currScene.key);
            }
        }
...
var GameSocketNew = (function () {
    ...
    GameSocketNew.prototype.connect = function (host, port) {
        Log.trace("com", "GameSocketNew.connect()");
        this.resetConnect();
        var ws = "wss://" + host + ":" + port;
        Log.trace("wss", "wss=" + ws);
    ...
    GameSocketNew.prototype.sendPackage = function (cmd, body) {
        if (!GameSocketNew.socket.connected) {
            Log.trace("com", "GameSocket.sendPackage(), 无连接");
            return;
        }
    ...
    GameSocketNew.prototype.connectHandler = function (e) {
        Log.trace("com", "GameSocket.connectHandler(), socket 连接成功");
    ...
    GameSocketNew.prototype.revSocketDataHandler = function (e) {
        Log.trace("com", "WebSocket receiveData");
        ...
        Log.trace("com", "packLen", packLen, "msgId", msgId);
        var bytes = new egret.ByteArray();
    ...
    GameSocketNew.prototype.excuteHandle = function (type, content) {
        switch (type) {
            ...
            case egret.Event.CLOSE:
                ...
                Log.trace("com", "GameSocket.excuteHandle(), 数据连接关闭");
                break;
            case egret.IOErrorEvent.IO_ERROR:
                // 连接错误处理
                this.listener.onDisConnect();
                Log.trace("com", "GameSocket.excuteHandle(), 数据连接异常!");
                break;
        }
    };
...
var GM = (function () {
    ...
    GM.init = function () {
        ...
        Log.trace("msg", "FrameworkVersion:" + BaseDefines.FrameworkVersion);
    };
    /** 收原生js信令 */
...
var App = (function () {
    ...
    Object.defineProperty(App, "VideoTools", {
        /**
         * m3u8视频播放
         * @returns {any}
         * @constructor
...
var BaseDefines = (function () {
    ...
    BaseDefines.FrameworkVersion = "0.0.3";
    return BaseDefines;
}());
...
var SoundBg = (function (_super) {
    ...
    SoundBg.prototype.play = function (effectName, volume, loops, end) {
        ...
        Log.trace("msg", "effectName:" + effectName);
...
var BigMath = (function () {
    ...
    BigMath.pow = function (e, i) {
        ...
        var r = "1";
        for (n = 2 * i, s = 2; Math.round(n) < 1 || Math.round(n) < 1 || Math.abs(Math.round(n) / s - i) > 1e-5;) {
            n *= 2, s *= 2;
        }
...
var SlideLine = (function (_super) {
    ...
    SlideLine.prototype.drawPoint = function (p1, p2) {
        var _this = this;
        // 	Log.trace("test","draw" + "p1=" + p1.toString() + "," + "p2=" + p2.toString());
        this.alpha = 1;
        var n = (p1.x + p2.x) * 0.5;
        var s = (p1.y + p2.y) * 0.5;
        this.graphics.clear();
...
var SlideLineManager = (function () {
    ...
    SlideLineManager.prototype.drawPoint = function (x, y) {
        if (this._recordPoint == null) {
            ...
            // Log.trace("test","distance=" + distance);
            if (distance > 50) {
 ...
 * 视频工具类
 */
var VideoTools = (function () {
    function VideoTools() {
        this.referenceVideoJs();
    }
    /**
     * 调用m3u8视频时为原生js 需要用到video.js插件
     * 播放m3u8视频时需要先引用video.js相关
     */
    VideoTools.prototype.referenceVideoJs = function () {
        var videoMinJs = document.createElement("script");
        videoMinJs.src = "./video-js/video.min.js";
        document.body.appendChild(videoMinJs);
        // tslint:disable-next-line: variable-name
        var videoJs_contrib_hls_min_js = document.createElement("script");
        videoJs_contrib_hls_min_js.src = "./video-js/videojs-contrib-hls.min.js";
        document.body.appendChild(videoJs_contrib_hls_min_js);
        // tslint:disable-next-line: variable-name
        var video_min_css = document.createElement("link");
        video_min_css.href = "./video-js/video-js.min.css";
        video_min_css.rel = "stylesheet";
        document.body.appendChild(video_min_css);
        var dolphinCss = document.createElement("link");
        dolphinCss.href = "./video-js/VideoStyle.css";
        dolphinCss.rel = "stylesheet";
        document.body.appendChild(dolphinCss);
    };
    /**
     * 播放m3u8视频用原生js video标签
     */
    VideoTools.prototype.videoTools_playM3u8 = function (videoId, width, height, videoUrl, bAuto) {
        if (bAuto === void 0) { bAuto = false; }
        var videoLab = document.createElement("video");
        videoLab.id = videoId;
        videoLab.setAttribute("class", "video-js vjs-default-skin vjs-big-play-centered");
        videoLab.setAttribute("controls", "");
        if (bAuto) {
            videoLab.setAttribute("autoplay", "");
        }
        videoLab.preload = "auto";
        videoLab.setAttribute("data-setup", "{}");
        videoLab.width = width;
        videoLab.height = height;
        var sourceLab = document.createElement("source");
        sourceLab.src = videoUrl;
        sourceLab.type = "application/x-mpegURL";
        videoLab.appendChild(sourceLab);
        document.body.appendChild(videoLab);
    };
    /**
     * 移除video标签
     */
    VideoTools.prototype.deleteVideoLab = function (videoId) {
        var video = document.getElementById(videoId);
        document.body.removeChild(video);
    };
    Object.defineProperty(VideoTools, "instance", {
        get: function () {
            if (!VideoTools._instance) {
                VideoTools._instance = new VideoTools();
            }
...
__reflect(VideoTools.prototype, "VideoTools");
/**
 * 闪烁特效类
 */
